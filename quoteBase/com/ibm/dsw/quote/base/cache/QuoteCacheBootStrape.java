 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class QuoteCacheBootStrape {
    
    private static QuoteCacheBootStrape instance = null;
    
    private static ApplicationProperties appProp = ApplicationProperties.getInstance();
    
    private TopazCacheableFactory[] keys = null;
    
    private QuoteCacheBootStrape(){}
    
    public static QuoteCacheBootStrape getInstance(){
        if(instance == null){
            instance = new QuoteCacheBootStrape();
        }
        return instance;
    } 
    ...
    public void initialize(){
        ...
        logCtx.debug(this,"Begin to Initialize QuoteCache.");
        CacheContext cc = CacheContextFactory.singleton().getCacheContext();
        logCtx.debug(this,"Initialization of CacheContext is finished.");
        try {
            Cache cache = cc.getCache();
            if(cache instanceof QuoteCache){
                logCtx.debug(this,"Begin to load cache group:QUOTE_CACHE_GROUP");
                ((QuoteCache)cache).loadGroup("QUOTE_CACHE_GROUP",keys);
            }else{
    ...
    private void loadInitConfigs(){
        LogContext logCtx = LogContextFactory.singleton().getLogContext();
        logCtx.debug(this,"Begin to load InitConfigs.");
        SAXBuilder builder = new SAXBuilder();
        InputStream in = Thread.currentThread().getContextClassLoader().getResourceAsStream("/portal-config/init-cache-config.xml");
        try {
        ...
        logCtx.debug(this,"Finished loading InitConfigs.");
    }
}
